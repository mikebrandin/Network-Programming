...
class TestSlowArrivalNoCorruptionNoLoss(unittest.TestCase):
    ...
    def test_corrupted_packet(self):
        ...
        altered_input_string = ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(4))
        pkt = pack("!HIHI%is" % len(input_string), 0b10000000, 48, checksum, len(input_string), altered_input_string.encode())
