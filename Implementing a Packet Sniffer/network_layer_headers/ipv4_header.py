...
class IPv4Header(LayerHeader):
    def __init__(self, pkt):
        ...
        ip_header = unpack('!BBHHHBBHII',  self.header_bytes)
        ...
        self.flags = ip_header[4] >> 3 
        self.fragment_offset = ip_header[4] & 8191
        self.TTL = ip_header[5]
        self.transport_protocol = ip_header[6]
        self.checksum = ip_header[7]
        self.source_addr = (ip_header[8])
        self.dest_addr = (ip_header[9])

        # TODO: You do not need to unpack any options, if they are present in the header. 
        # However, if options are present, store the bytes associated with them in self.options_bytes.
        if (self.IHL  > 5):
